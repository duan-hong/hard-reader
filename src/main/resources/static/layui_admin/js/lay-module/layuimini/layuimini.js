...
layui.define(["element", "jquery"], function (exports) {
    ...
    var baseUrl_hard = "http://127.0.0.1:8082/hard-reader/page/";
    var baseUrl_flow = "http://127.0.0.1:8088/flow-monitor/page/";
    var baseUrl_pro = "http://127.0.0.1:8087/processes-manager/page/";
    var imgBaseUrl = "http://127.0.0.1:8082/hard-reader/layui_admin/";
    ...
    layuimini = new function () {
        ...
        this.initHome = function (data) {
            ...
            if (data.sysId==='hard-reader'){
                $('#layuiminiHomeTabIframe').html('<iframe width="100%" height="100%" frameborder="0"  src="' + baseUrl_hard + data.href + '"></iframe>');
            }else if (data.sysId==='flow-monitor'){
                $('#layuiminiHomeTabIframe').html('<iframe width="100%" height="100%" frameborder="0"  src="' + baseUrl_flow + data.href + '"></iframe>');
            }else if (data.sysId==='processes-manager'){
                $('#layuiminiHomeTabIframe').html('<iframe width="100%" height="100%" frameborder="0"  src="' + baseUrl_pro + data.href + '"></iframe>');
            }
        };
        ...
        this.initLogo = function (data) {
            var html = '<a href="' + data.href + '">\n' +
                '<img src="' + imgBaseUrl + data.image + '" alt="logo">\n' +
                '<h1>' + data.title + '</h1>\n' +
                '</a>';
            ...
            $.each(data, function (key, val) {
                ...
                $.each(menuList, function (index, menu) {
                    ...
                    if (menu.child != undefined && menu.child != []) {
                        ...
                        var buildChildHtml = function (html, child, menuParameId) {
                            ...
                            $.each(child, function (childIndex, childMenu) {
                                ...
                                if (childMenu.child != undefined && childMenu.child != []) {
                                    ...
                                    html += '<a href="javascript:;" class="layui-menu-tips" data-type="tabAdd"  data-tab-mpi="m-p-i-' + menuParameId + '" data-tab="' + childMenu.href + '" data-sys-id="'+childMenu.sysId+'" target="' + childMenu.target + '"><i class="' + childMenu.icon + '"></i><span class="layui-left-nav"> ' + childMenu.title + '</span></a>\n';
                                    menuParameId++;
                        ...
                        leftMenuHtml += '<a href="javascript:;" class="layui-menu-tips"  data-type="tabAdd" data-tab-mpi="m-p-i-' + menuParameId + '" data-tab="' + menu.href + '"  data-sysId="'+menu.sysId+'" target="' + menu.target + '"><i class="' + menu.icon + '"></i><span class="layui-left-nav"> ' + menu.title + '</span></a>\n';
                        menuParameId++;
                    }
                    leftMenuHtml += '</li>\n';
                });
    ...
    $('body').on('click', '[data-tab]', function () {
        ...
        var tabId = $(this).attr('data-tab'),
            ...
            currentSysId=$(this).attr("data-sys-id"),
            title = $(this).html(),
        ...
        if (layuimini.config('urlSuffixDefault')) {
            ...
            var tabIdTemp='';
            if (href.indexOf("?") > -1) {
                tabIdTemp = href + '&mpi=' + menuParameId;
                tabId = tabIdTemp;
            } else {
                tabIdTemp = href;
                tabId = tabIdTemp;
        ...
        //判断链接归属的系统
        if (currentSysId==='hard-reader'){
            href=baseUrl_hard+ href;
        }else if (currentSysId==='flow-monitor'){
            href=baseUrl_flow+ href;
        }else if (currentSysId==='processes-manager'){
            href=baseUrl_pro+ href;
        }
        console.log(href)
